#include <array>
#include <cmath>
#include <SFML/Graphics.hpp>

#include "../Global.hpp"
#include "Pacman.hpp"
#include "../MapCollision/MapCollision.hpp"

/**
 * Konstruktor klasy Pacman
 * Inicjalizuje podstawowe wartości:
 * - stan animacji
 * - stan życia
 * - kierunek
 * - timer energizera
 * - pozycję początkową
 */
Pacman::Pacman() :
	animation_over(0),
	dead(0),
	direction(0),
	energizer_timer(0),
	position({0, 0})
{
}

/**
 * Sprawdza czy animacja została zakończona
 * @return true jeśli animacja się zakończyła, false w przeciwnym razie
 */
bool Pacman::get_animation_over()
{
	return animation_over;
}

/**
 * Sprawdza czy Pacman jest martwy
 * @return true jeśli Pacman jest martwy, false w przeciwnym razie
 */
bool Pacman::get_dead()
{
	return dead;
}

/**
 * Pobiera aktualny kierunek ruchu Pacmana
 * @return Kierunek (0-prawo, 1-góra, 2-lewo, 3-dół)
 */
unsigned char Pacman::get_direction()
{
	return direction;
}

/**
 * Pobiera pozostały czas działania energizera
 * @return Pozostały czas działania energizera
 */
unsigned short Pacman::get_energizer_timer()
{
	return energizer_timer;
}

/**
 * Rysuje Pacmana na ekranie
 * Obsługuje różne stany animacji (normalna, śmierć, zwycięstwo)
 * @param i_victory Czy Pacman wygrał poziom
 * @param i_window Referencja do okna SFML
 */
void Pacman::draw(bool i_victory, sf::RenderWindow& i_window)
{
	unsigned char frame = static_cast<unsigned char>(floor(animation_timer / static_cast<float>(PACMAN_ANIMATION_SPEED)));

	sf::Sprite sprite;

	sf::Texture texture;

	sprite.setPosition(position.x, position.y);

	if (1 == dead || 1 == i_victory)
	{
		if (animation_timer < PACMAN_DEATH_FRAMES * PACMAN_ANIMATION_SPEED)
		{
			animation_timer++;

			texture.loadFromFile("Resources/PacmanDeath" + std::to_string(CELL_SIZE) + ".png");

			sprite.setTexture(texture);
			sprite.setTextureRect(sf::IntRect(CELL_SIZE * frame, 0, CELL_SIZE, CELL_SIZE));

			i_window.draw(sprite);
		}
		else
		{
		
			animation_over = 1;
		}
	}
	else
	{
		texture.loadFromFile("Resources/Pacman" + std::to_string(CELL_SIZE) + ".png");

		sprite.setTexture(texture);
		sprite.setTextureRect(sf::IntRect(CELL_SIZE * frame, CELL_SIZE * direction, CELL_SIZE, CELL_SIZE));

		i_window.draw(sprite);

		animation_timer = (1 + animation_timer) % (PACMAN_ANIMATION_FRAMES * PACMAN_ANIMATION_SPEED);
	}
}

/**
 * Resetuje stan Pacmana do wartości początkowych
 * Używane przy rozpoczęciu nowej gry lub nowego poziomu
 */
void Pacman::reset()
{
	animation_over = 0;
	dead = 0;

	direction = 0;

	animation_timer = 0;
	energizer_timer = 0;
}

/**
 * Ustawia timer animacji
 * @param i_animation_timer Nowa wartość timera animacji
 */
void Pacman::set_animation_timer(unsigned short i_animation_timer)
{
	animation_timer = i_animation_timer;
}

/**
 * Ustawia stan śmierci Pacmana
 * @param i_dead Nowy stan śmierci
 */
void Pacman::set_dead(bool i_dead)
{
	dead = i_dead;

	if (1 == dead)
	{
	
		animation_timer = 0;
	}
}

/**
 * Ustawia pozycję Pacmana na mapie
 * @param i_x Pozycja X
 * @param i_y Pozycja Y
 */
void Pacman::set_position(short i_x, short i_y)
{
	position = {i_x, i_y};
}

/**
 * Aktualizuje stan Pacmana
 * Obsługuje:
 * - Kolizje ze ścianami
 * - Ruch w odpowiedzi na klawisze
 * - Przechodzenie przez tunel
 * - Zbieranie energizerów
 * @param i_level Aktualny poziom gry
 * @param i_map Referencja do mapy gry
 */
void Pacman::update(unsigned char i_level, std::array<std::array<Cell, MAP_HEIGHT>, MAP_WIDTH>& i_map)
{
	std::array<bool, 4> walls{};
	walls[0] = map_collision(0, 0, PACMAN_SPEED + position.x, position.y, i_map);
	walls[1] = map_collision(0, 0, position.x, position.y - PACMAN_SPEED, i_map);
	walls[2] = map_collision(0, 0, position.x - PACMAN_SPEED, position.y, i_map);
	walls[3] = map_collision(0, 0, position.x, PACMAN_SPEED + position.y, i_map);

	if (1 == sf::Keyboard::isKeyPressed(sf::Keyboard::Right))
	{
		if (0 == walls[0]) 
		{
			direction = 0;
		}
	}

	if (1 == sf::Keyboard::isKeyPressed(sf::Keyboard::Up))
	{
		if (0 == walls[1])
		{
			direction = 1;
		}
	}

	if (1 == sf::Keyboard::isKeyPressed(sf::Keyboard::Left))
	{
		if (0 == walls[2])
		{
			direction = 2;
		}
	}

	if (1 == sf::Keyboard::isKeyPressed(sf::Keyboard::Down))
	{
		if (0 == walls[3])
		{
			direction = 3;
		}
	}

	if (0 == walls[direction])
	{
		switch (direction)
		{
			case 0:
			{
				position.x += PACMAN_SPEED;

				break;
			}
			case 1:
			{
				position.y -= PACMAN_SPEED;

				break;
			}
			case 2:
			{
				position.x -= PACMAN_SPEED;

				break;
			}
			case 3:
			{
				position.y += PACMAN_SPEED;
			}
		}
	}

	if (-CELL_SIZE >= position.x)
	{
		position.x = CELL_SIZE * MAP_WIDTH - PACMAN_SPEED;
	}
	else if (CELL_SIZE * MAP_WIDTH <= position.x)
	{
		position.x = PACMAN_SPEED - CELL_SIZE;
	}

	if (1 == map_collision(1, 0, position.x, position.y, i_map)) 
	{
		
		energizer_timer = static_cast<unsigned short>(ENERGIZER_DURATION / pow(2, i_level));
	}
	else
	{
		energizer_timer = std::max(0, energizer_timer - 1);
	}
}

Position Pacman::get_position()
{
	return position;
}
