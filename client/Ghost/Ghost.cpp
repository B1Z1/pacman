...
#include "../MapCollision/MapCollision.hpp"

/**
 * Konstruktor klasy Ghost inicjalizujący ducha z określonym identyfikatorem
 * @param i_id Identyfikator ducha (0-3)
 */
Ghost::Ghost(unsigned char i_id) : id(i_id) {
}

/**
 * Sprawdza kolizję ducha z Pacmanem
 * @param i_pacman_position Pozycja Pacmana do sprawdzenia
 * @return true jeśli nastąpiła kolizja, false w przeciwnym razie
 */
...
/**
 * Oblicza odległość od celu dla zadanego kierunku ruchu
 * @param i_direction Kierunek ruchu (0-prawo, 1-góra, 2-lewo, 3-dół)
 * @return Odległość od celu po wykonaniu ruchu w danym kierunku
 */
 ...
 * @param i_flash Parametr określający czy duch ma migać
 * @param i_window Okno renderowania SFML
 */
void Ghost::draw(bool i_flash, sf::RenderWindow &i_window) {
    unsigned char body_frame = static_cast<unsigned char>(floor(
        animation_timer / static_cast<float>(GHOST_ANIMATION_SPEED)));
...
/**
 * Resetuje ducha do pozycji startowej
 * @param i_home Pozycja domu ducha
 * @param i_home_exit Pozycja wyjścia z domu ducha
 */
...
/**
 * Ustawia pozycję ducha
 * @param i_x Nowa pozycja X
 * @param i_y Nowa pozycja Y
 */
 ...
 * @param i_level Poziom trudności
 * @param i_map Mapa gry
 * @param i_ghost_0 Inny duch (używane do obliczeń)
 * @param i_pacman Obiekt Pacmana
 */
...
/**
 * Aktualizuje cel ducha w zależności od trybu ruchu i pozycji Pacmana
 * @param i_pacman_direction Kierunek ruchu Pacmana
 * @param i_ghost_0_position Pozycja innego ducha
 * @param i_pacman_position Pozycja Pacmana
 */
    ...
    if (1 == use_door) {
        ...
        if (0 == movement_mode) {
            ...
            switch (id) {
                ...
                case 3: {
                    if (CELL_SIZE * GHOST_3_CHASE <= sqrt(
                            pow(position.x - i_pacman_position.x, 2) + pow(position.y - i_pacman_position.y, 2))) {
                        target = i_pacman_position;
                    } else {
                        target = { 0, CELL_SIZE * (MAP_HEIGHT - 1) };
                    }
...
/**
 * Zwraca aktualną pozycję ducha
 * @return Pozycja ducha
 */
